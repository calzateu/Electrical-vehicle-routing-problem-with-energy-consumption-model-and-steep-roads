...
class ConstructiveMethod4:
    def __init__(self, data) -> None:
        ...
        self.reference_nodes = data.reference_nodes
        self.node_to_index = data.node_to_index
        self.angles = data.angles
        self.distances_dict = data.distances
        ...
        while missing_nodes > 0:
            for i in self.__order_vehicles(traveled_distances):  # Hacer un foreach ordenando el recorrido
                ...
                while not stop:
                    ...
                    # energy_between_a_b(angles[(u, v)], distances[(u, v)], information)
                    self.information["m"] = new_capacity
                    keys = list(self.reference_nodes.keys())
                    past_difference = self.reference_nodes[keys[0]] - self.reference_nodes[keys[1]]
                    new_difference = energy_between_a_b(self.angles[keys[0]], self.distances_dict[keys[0]], self.information) - \
                                     energy_between_a_b(self.angles[keys[1]], self.distances_dict[keys[1]], self.information)
                    print(past_difference != new_difference, past_difference, new_difference)
